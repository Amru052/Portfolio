...
export default function Experience() {
    ...
    return (
        <section id="experience" ref={containerRef} className="py-24 px-6 md:px-12 max-w-6xl mx-auto z-10 relative bg-background">
            <div className="mb-20">
                <h2 className="font-heading font-bold text-4xl md:text-6xl text-dark mb-6 tracking-tight">Active Deployments.</h2>
            ...
            <div className="relative pl-6 md:pl-10">
                ...
                <div className="absolute top-0 left-[11px] md:left-[27px] w-px h-full bg-dark/10"></div>
                ...
                <div className="flex flex-col gap-16">
                    {experiences.map((exp, i) => (
                        <div key={i} className="exp-item relative group">
                            {/* Timeline nodes */}
                            <div className="absolute -left-6 md:-left-10 w-3 h-3 rounded-full bg-paper border-2 border-accent group-hover:bg-accent transition-colors duration-300 transform -translate-x-1/2 mt-2 z-10"></div>

                            <div className="flex flex-col lg:flex-row gap-6 lg:gap-12 pl-6">
                                <div className="lg:w-1/3 flex flex-col items-start gap-3">
                                    <span className="font-data text-xs text-paper uppercase tracking-widest bg-dark px-3 py-1 rounded">
                                        {exp.date}
                                    </span>
                                    <div>
                                        <h3 className="font-heading font-bold text-xl md:text-2xl text-dark leading-tight group-hover:text-accent transition-colors duration-300">
                                            {exp.role}
                                        </h3>
                                        <p className="font-drama italic text-lg text-dark/80 mt-1">
                                            {exp.company}
                                        </p>
                                        <p className="font-data text-[10px] text-dark/50 tracking-widest uppercase mt-3">
                                            [ {exp.location} ]
                                        </p>
                                    </div>
                                </div>

                                <div className="lg:w-2/3 bg-paper/50 p-6 md:p-8 rounded-xl border border-dark/5 shadow-inner">
